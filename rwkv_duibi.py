...
class RWKV_UNet_encoder(nn.Module):
    def __init__(self, dim_in=3, num_classes=1000, img_size=224,
                 ...
                 dw_kss=[3, 3, 1, 1], se_ratios=[0.0, 0.0, 0.0, 0.0], attn_ss=[False, False, True, True], drop=0.,
                 drop_path=0., pre_dim=0, channel_gamma=1 / 4, shift_pixel=1):
        super().__init__()
...
def RWKV_UNet_encoder_T(pretrained=False, **kwargs):
    model = RWKV_UNet_encoder(
        ...
        depths=[2, 2, 4, 2], stem_dim=24, embed_dims=[32, 48, 96, 160], exp_ratios=[2., 2.5, 3.0, 3.5],
        norm_layers=['bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'], act_layers=['silu', 'silu', 'gelu', 'gelu'],
        dw_kss=[5, 5, 5, 5], attn_ss=[False, False, True, True], drop=0., drop_path=0.05, **kwargs)
    return model
...
def RWKV_UNet_encoder_S(pretrained=False, **kwargs):
    model = RWKV_UNet_encoder(
        # dim_in=3, num_classes=1000, img_size=224,
        depths=[3, 3, 6, 3], stem_dim=24, embed_dims=[32, 64, 128, 192], exp_ratios=[2., 2.5, 3.0, 4.0],
        norm_layers=['bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'], act_layers=['silu', 'silu', 'gelu', 'gelu'],
        dw_kss=[5, 5, 5, 5], attn_ss=[False, False, True, True], drop=0., drop_path=0.05, **kwargs)
    return model
...
def RWKV_UNet_encoder_B(pretrained=False, **kwargs):
    model = RWKV_UNet_encoder(
        # dim_in=3, num_classes=1000, img_size=224,
        depths=[3, 3, 6, 3], stem_dim=24, embed_dims=[48, 72, 144, 240], exp_ratios=[2., 2.5, 4.0, 4.0],
        norm_layers=['bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'], act_layers=['silu', 'silu', 'gelu', 'gelu'],
        dw_kss=[5, 5, 5, 5], attn_ss=[False, False, True, True], drop=0., drop_path=0.05, **kwargs)
    return model
...
from uncertain_duibi import AuxiliaryHead
...
class RWKV_UNet(nn.Module):
    def __init__(self, in_channels=1, num_classes=9, img_size=224, pretrained_path='net_B.pth'):
        ...
        self.decoder1 = UpBlock(self.embed_dims[3], self.embed_dims[2], norm_in=False, has_skip=False, exp_ratio=1.0,
                                dw_ks=9, stride=1, dilation=1, se_ratio=0.0, drop_path=0., drop=0.)
        self.decoder2 = UpBlock(self.embed_dims[2] * 2, self.embed_dims[1], norm_in=False, has_skip=False,
                                exp_ratio=1.0, dw_ks=9, stride=1, dilation=1, se_ratio=0.0, drop_path=0., drop=0.)
        self.decoder3 = UpBlock(self.embed_dims[1] * 2, self.embed_dims[0], norm_in=False, has_skip=False,
                                exp_ratio=1.0, dw_ks=9, stride=1, dilation=1, se_ratio=0.0, drop_path=0., drop=0.)
        self.decoder4 = UpBlock(self.embed_dims[0] * 2, 24, norm_in=False, has_skip=False, exp_ratio=1.0, dw_ks=9,
                                stride=1, dilation=1, se_ratio=0.0, drop_path=0., drop=0.)
        self.final_conv = nn.Conv2d(24, num_classes, kernel_size=1)

        self.outputs1 = nn.Conv2d(72, 9, kernel_size=1, padding=0)
        self.outputs = nn.Conv2d(48, 9, kernel_size=1, padding=0)
        self.aux_head = AuxiliaryHead(240, 128, 64)
    ...
    def forward(self, x):
        ...
        mask_fg, mask_uc = self.aux_head(x)

        _, dec3 = self.decoder1(x)
        d3, dec2 = self.decoder2(torch.cat([dec3, enc3], dim=1))
        d2, dec1 = self.decoder3(torch.cat([dec2, enc2], dim=1))
        _,dec0 = self.decoder4(torch.cat([dec1, enc1], dim=1))
        # Final output
        out = self.final_conv(dec0)

        au1 = self.outputs1(d3)
        au2 = self.outputs(d2)

        out1 = F.interpolate(au1, scale_factor=8, mode='bilinear')
        out2 = F.interpolate(au2, scale_factor=4, mode='bilinear')

        mask_fg = F.interpolate(mask_fg, scale_factor=16, mode='bilinear')

        mask_uc = F.interpolate(mask_uc, scale_factor=16, mode='bilinear')
        return out, out2, out1, mask_fg, mask_uc, d3, d2,au2, au1
